...
def main():
    ...
    st.sidebar.subheader("Choose Clasifier")
    classifier = st.sidebar.selectbox("Classifier",("Support Vector Machine (SMV)","Logistic Regression","Random Forest"))

    if classifier== 'Support Vector Machine (SMV)':
        ...
        if st.sidebar.button("Classify",key='classify'):
            st.subheader("Support Vector Machine (SMV) Results")
            model=SVC(C=C,kernel=kernel,gamma=gamma)
            model.fit(x_train, y_train)
            accuracy=model.score(x_test,y_test)
            y_pred=model.predict(x_test)
            st.write("Accucary :",accuracy.round(2))
    ...
    st.subheader("Created by Juan de Dios Huarancca")
